...
VelocityOrPressure = True  ########################################首先要进行判断
...
for count_Total_Clip_Number in range(0, total_Clip_Number):
    ...
    shutil.copyfile(Clip_Whole_Dir + Stored_Example_J_middle_dir + linuxShName,
                    Clip_Whole_Dir + Specified_Dir + 'B_' + linuxShName)
    # linuxSh = open(Clip_Whole_Dir + Stored_Example_J_middle_dir + linuxShName, "a+", encoding="utf-8")

    # 命名更改 复制
    for k in totalDivideList[count_Total_Clip_Number]:

...
#Data_Whole_Dir = os.path.abspath(os.path.dirname(Clip_Whole_Dir))



Specified_Dir_preffix_1 = "task"+"\\" +"0806" + "\\"  ###########!!!!!!!!!!
Specified_Dir_preffix_2 = 'MeshNum100' + '_'  ###########!!!!!!!!!!!
...
Mesh_load_dir_preffix = 'basic_Mesh'+'\\'+'0803_jiaodu'+'\\' #存储地址
for i in catchList:
    MeshName_Preffix_STR = 'Model_100'
    MeshName_Preffix = MeshName_Preffix_STR + '-' +str(i)
    ...
    B_J_Content_Preffix = 'call fluent 3ddp -t75 -g -wait -i '
    B_C_Content_Preffix = "call cfdpost -batch "

    # bat文件 和 cse文件
    B_J_Name = 'BF_' + Mesh_num_preffix + str(varNum_P) + "_" + str(varNum_V) + "_"+ MeshName_Preffix_STR + ".bat"
    B_J_Dir = Clip_Whole_Dir + Specified_Dir + B_J_Name
    B_C_Name = 'BP_' + Mesh_num_preffix + str(varNum_P) + "_" + str(varNum_V) + "_"+ MeshName_Preffix_STR + ".bat"
    ...
    # 命名更改 复制
    #for k in catchList:
    B_J_Middle_Name = B_J_middle_name_preffix + ".jou"
    B_J_Exported_Dir = Clip_Whole_Dir + Specified_Dir + B_J_Middle_Name  #####################命名jou等中间文件名称
    B_C_Middle_Name = B_C_middle_name_preffix  + ".cse"
